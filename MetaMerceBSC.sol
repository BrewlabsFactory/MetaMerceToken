...
contract MetaMerce is IERC20, Ownable {
	...
	uint256 private constant _circulatingSupply = 0;
	...
	event SwapAndLiquify(uint256 tokensSwapped, uint256 bscReceived,uint256 tokensIntoLiqudity);
	...
	function setBaseFeesOnBuy(uint8 _liquidityFeeOnBuy, uint8 _marketingFeeOnBuy, uint8 _buyBackFeeOnBuy, uint8 _holdersFeeOnBuy) external onlyOwner {
		require(_launchStartTimestamp > 0 && (block.timestamp - _launchStartTimestamp) > 1210000, "MetaMerce: Cannot change taxes within the first 14days of launch");
		require((_liquidityFeeOnBuy+_marketingFeeOnBuy+_buyBackFeeOnBuy+_holdersFeeOnBuy+_base.liquidityFeeOnSell+_base.marketingFeeOnSell+_base.buyBackFeeOnSell+_base.holdersFeeOnSell) <= 25, "MetaMerce: Buy and sell taxes cannot exceed 25%");
	...
	function setBaseFeesOnSell(uint8 _liquidityFeeOnSell,uint8 _marketingFeeOnSell, uint8 _buyBackFeeOnSell, uint8 _holdersFeeOnSell) external onlyOwner {
		require(_launchStartTimestamp > 0 && (block.timestamp - _launchStartTimestamp) > 1210000, "MetaMerce: Cannot change taxes within the first 14days of launch");
		require((_liquidityFeeOnSell+_marketingFeeOnSell+_buyBackFeeOnSell+_holdersFeeOnSell+_base.liquidityFeeOnBuy+_base.marketingFeeOnBuy+_base.buyBackFeeOnBuy+_base.holdersFeeOnBuy) <= 25, "MetaMerce: Buy and sell taxes cannot exceed 25%");
	...
	function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
		...
		require(newValue > getCirculatingSupply() * 100 / 100000, "MetaMerce: Cannot update maxTxAmount to <0.1% of supply");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "MetaMerce: Cannot update maxWalletAmount to same value");
		require(newValue > getCirculatingSupply() * 100 / 10000, "MetaMerce: Cannot update maxWalletAmount to <1% of supply");
	...
	function claimBSCOverflow(uint256 amount) external onlyOwner {
	...
	function getCirculatingSupply() public view returns (uint256) {
		uint256 burntBalance = balanceOf(address(0x0)) + balanceOf(address(0x000000000000000000000000000000000000dEaD));
		return _tTotal - burntBalance;
	}
	...
	function _swapAndLiquify() private {
		...
		uint256 initialBSCBalance = address(this).balance;
		...
		_swapTokensForBSC(amountToSwap);

		uint256 bscBalanceAfterSwap = address(this).balance - initialBSCBalance;
		uint256 totalBSCFee = totalFeePrior - (liquidityFeePrior / 2) - (holdersFeePrior);
		uint256 amountBSCLiquidity = bscBalanceAfterSwap * liquidityFeePrior / totalBSCFee / 2;
		uint256 amountBSCMarketing = bscBalanceAfterSwap * marketingFeePrior / totalBSCFee;
		uint256 amountBSCBuyBack = bscBalanceAfterSwap - (amountBSCLiquidity + amountBSCMarketing);

		Address.sendValue(payable(marketingWallet),amountBSCMarketing);
        Address.sendValue(payable(buyBackWallet),amountBSCBuyBack);

		if (amountToLiquify > 0) {
			_addLiquidity(amountToLiquify, amountBSCLiquidity);
			emit SwapAndLiquify(amountToSwap, amountBSCLiquidity, amountToLiquify);
		}
	...
	function _swapTokensForBSC(uint256 tokenAmount) private {
		...
		uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
			tokenAmount,
			1, // accept any amount of BSC
			path,
			address(this),
			block.timestamp
		);
	}
	function _addLiquidity(uint256 tokenAmount, uint256 bscAmount) private {
		_approve(address(this), address(uniswapV2Router), tokenAmount);
		uniswapV2Router.addLiquidityETH{value: bscAmount}(
			address(this),
			tokenAmount,
